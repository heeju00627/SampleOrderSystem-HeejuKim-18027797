...
```
→ reserved 주문 목록 표시 (orderId | 시료ID | 고객명 | 주문수량)
→ 처리할 주문 ID 입력 (없으면 메인 복귀)
→ [Y] 승인 / [N] 거절 선택

  승인:
    재고 충분 →
      [OK] 승인 완료 — 출고 대기 (confirmed)
           시료: S0001 / 차감 재고: 50개 → 잔여: 20개

    재고 부족 →
      [OK] 승인 완료 — 생산 등록 (producing)
           시료: S0001 / 실생산량: 33개 / 예상완료: 2026-05-08T14:30:00
...
5-1. 현재 생산 중인 주문
     → 없으면 "(생산 중인 주문 없음)" 출력
     → 있으면:
        orderId | 시료 | 고객 | 수량 | 시작시각 | 예상완료 | 잔여시간

     잔여시간 표시: "Xh Ym" (60분 이상) 또는 "Ym" (60분 미만)
     잔여시간 색상:
       60분 초과 → Green
       60분 이하 → Yellow
       0분 (완료 대기) → Red
...
→ Y 선택 시:
   [OK] 출고 완료: ORD-20260508-0001 (released)
→ N 선택 시: 취소
```
